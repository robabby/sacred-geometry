...
export function MetatronsCube() {
  const renderCircles = () =>
    Object.entries(CIRCLES).map(([key, d], index) => (
      <motion.path
        key={key}
        ...
        animate={{
          opacity: 1,
          transition: { duration: 0.25, delay: index * 0.075 },
        }}
        style={{ fill: "none" }}
        id={`${key}-circle`}
      />
  ...
  return (
    ...
    >
      <motion.g id="layer1">
        <motion.g id="circles">{renderCircles()}</motion.g>
        <motion.g id="lines">
          <motion.path
            stroke="var(--amber-a10)"
